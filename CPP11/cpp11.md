...
* 类型中的trivial意义：  
    - 没有用户定义的构造函数、析构函数或拷贝/移动运算符
    - 默认构造函数：该类型有一个 trivial 的默认构造函数。也就是说，编译器可以为该类型生成一个构造函数，该构造函数不会执行任何初始化（例如，对于 POD 类型，所有成员变量都将不会被初始化）。

    - 拷贝构造函数：该类型有一个 trivial 的拷贝构造函数，意味着编译器生成的拷贝构造函数仅仅是逐位复制其成员。

    - 移动构造函数：该类型有一个 trivial 的移动构造函数，表示编译器生成的移动构造函数仅仅是逐位复制其成员。

    - 析构函数：该类型有一个 trivial 的析构函数，意味着这个析构函数不执行任何特殊的清除操作。

    - 拷贝赋值运算符：该类型有一个 trivial 的拷贝赋值运算符，表示这个运算符仅仅是逐位复制其成员。

    - 移动赋值运算符：该类型有一个 trivial 的移动赋值运算符，仅仅是逐位复制其成员。

    - 没有虚函数：该类型没有任何虚函数。
    - POD类型：在 C++ 中，POD（Plain Old Data）类型是一种简单的数据类型，其行为类似于 C 语言中的结构体或联合体。POD 类型的定义在 C++11 之前主要依赖于标准库的约定，而在 C++11 中，POD 类型被明确分为两类：Trivial types 和 Standard-layout types。
        1. Trivial Type：该类型是 trivial 的，意味着：
            - 它有一个 trivial 的默认构造函数。
            - 它有一个 trivial 的拷贝构造函数。
            - 它有一个 trivial 的移动构造函数（如果有）。
            - 它有一个 trivial 的析构函数。
            - 它有一个 trivial 的拷贝赋值运算符。
            - 它有一个 trivial 的移动赋值运算符（如果有）。
            - 它没有虚函数。
        2. Standard-layout Type：该类型的布局必须符合标准布局的规则，意味着：
            - 所有非静态数据成员的访问控制（public、protected、private）必须相同。
            - 所有非静态数据成员的类型必须是标准布局类型。
            - 该类型不能有虚基类。
            - 该类型的基类必须是标准布局类型（如果有）。
            - 该类型的成员只能有一个基类。
        3. 作用：
            - 与 C 代码兼容性：POD 类型与 C 语言的数据结构相似，可以方便地与 C 代码进行交互。
            - 高效的内存操作：由于 POD 类型的简单性，可以使用 memcpy 等低级内存操作来对其进行快速复制和移动。
            - 序列化和反序列化：POD 类型的简单结构使其易于在网络传输或文件存储中进行序列化和反序列化。
