...
void resolveUsbInfo(UsbDevice* usbDevice, uint16_t& pid, uint16_t& vid, uint16_t& cid, bool resolveAll) {
...
QPair<uint16_t, uint16_t> getDevPidVid(libusb_device *dev) {
    QPair<uint16_t, uint16_t> res{0, 0};
    struct libusb_device_descriptor desc;
    int r = libusb_get_device_descriptor(dev, &desc);
    if (r >= 0) {
        res.first = desc.idProduct;
        res.second = desc.idVendor;
    }
    return res;
}
